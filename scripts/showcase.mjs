...
// before the tour actually starts (recorded below) off the front in ffmpeg. (5b)
...
// that element has actually PAINTED (video has a decoded frame / img is loaded)
// before recording, so the tour never runs against the fallback. See step 5b.
if (cfg.waitBg)
  await page
    .waitForFunction(
      (sel) => {
...
// Everything before this instant is blank-page loading — trim it off in ffmpeg. (5b)
